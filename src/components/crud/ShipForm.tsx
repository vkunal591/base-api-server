...
import { ShipType } from "./formInput/shipsFormType";
...
const ShipForm: React.FC<LedgerProps> = (props: any) => {
  const data = props.data;
  const formType = props.formType;
  const [loading, setLoading] = useState(true);
  const [submitting, setSubmitting] = useState(false);
  const [formField, setFormFields] = useState<any>(
    data?._id ? populateFormFields(ShipType, data) : ShipType

  );
  const [formData, setFormData] = useState<any>(
    data?._id ? populateFormData(ShipType, data) : {}
  );
  ...
  const makeApiCall = async (updatedData: any) => {
    try {
      ...
      const response: any = data?._id
        ...
        : await Post(`https://sud-group-dock.vercel.app/${url}`, updatedData);
